...
const REAL_MODELS: Record<string,{provider:string;model:string}> = {qwen:{provider:'qwen',model:'qwen-plus'}}
...
export default function App(){
  ...
  return <div className="app-shell">
    ...
    <main>
      ...
      {tab==='cases'&&<Cases items={data.testCases} results={data.results} setItems={(items:TestCase[])=>update({...data,testCases:items})} notify={notify}/>} 
      {tab==='prompts'&&<Prompts items={data.promptVersions} results={data.results} setItems={(items:PromptVersion[])=>update({...data,promptVersions:items})} notify={notify}/>} 
      {tab==='runner'&&<Runner data={data} save={(r:EvalResult)=>{update({...data,results:[r,...data.results]});notify('评测记录已保存')}}/>}
...
function Runner({data,save}:any){
 ...
 const [modelId,setModel]=useState(data.models[0]?.id||'')
 ...
 const model=data.models.find((x:any)=>x.id===modelId)
 const realConfig=REAL_MODELS[modelId]
 const realUnavailable=runMode==='real'&&!realConfig
 const resetRun=()=>{setStage('idle');setOutput('');setJudgement(null);setUsage(undefined);setApiIdentity(undefined);setError('')}
 const run=async()=>{
   if(!tc||!pv||!model||realUnavailable)return
   setStage('running');setError('');setJudgement(null);setUsage(undefined)
   if(runMode==='mock'){
   ...
   try{
     const response=await requestChat({provider:realConfig.provider,model:realConfig.model,systemPrompt:pv.content,userInput:tc.inputText,temperature})
     setOutput(response.output);setLatency(response.latencyMs);setUsage(response.usage);setApiIdentity({provider:response.provider,model:response.model});setStage('output')
   }catch(err){setError(err instanceof Error?err.message:'真实模型请求失败。');setStage('idle')}
 }
 const judge=()=>{setStage('judging');setTimeout(()=>{const base=76+Math.floor(Math.random()*18);const d={accuracy:Math.min(100,base+5),completeness:base,format:Math.min(100,base+3),usefulness:Math.min(100,base+2),safety:Math.min(100,base+10)};setJudgement({score:avg(Object.values(d) as number[]),dimensionScores:d,badCaseType:'无',judgeComment:'模型输出与任务目标一致，关键事实处理准确，整体表达清晰。',suggestion:'可进一步压缩冗余表述，并严格对齐期望答案中的字段结构。'});setStage('judged')},900)}
 const doSave=()=>{save({id:uid('er'),testCaseId:tcId,promptVersionId:pvId,testCaseTitleSnapshot:tc.title,testCaseInputSnapshot:tc.inputText,expectedAnswerSnapshot:tc.expectedAnswer,evaluationCriteriaSnapshot:tc.evaluationCriteria,promptVersionNameSnapshot:pv.name,promptContentSnapshot:pv.content,modelId,modelName:model.name,provider:apiIdentity?.provider,apiModel:apiIdentity?.model,usage,modality:tc.modality,runMode,modelOutput:output,latencyMs:latency,...judgement,createdAt:new Date().toISOString()});setStage('saved')}
 if(!tc||!pv||!model)return <><PageHead eyebrow="EVALUATION PLAYGROUND" title="Prompt 测试台" desc="组合用例、Prompt 和模型，运行 Mock 或真实 API 评测。"/><Empty text="请先准备至少一条测试用例、一个 Prompt 版本和一个模型"/></>
 ...
 <div className="runner-config"><label><span>01 测试用例</span><select value={tcId} onChange={e=>{setTc(e.target.value);resetRun()}}>{data.testCases.map((x:TestCase)=><option value={x.id} key={x.id}>{x.title}</option>)}</select></label><label><span>02 Prompt 版本</span><select value={pvId} onChange={e=>{setPv(e.target.value);resetRun()}}>{data.promptVersions.map((x:PromptVersion)=><option value={x.id} key={x.id}>{x.name} · {x.taskType}</option>)}</select></label><label><span>03 模型</span><select value={modelId} onChange={e=>{setModel(e.target.value);resetRun()}}>{data.models.map((x:any)=><option value={x.id} key={x.id}>{x.name}</option>)}</select></label>{runMode==='real'&&<label><span>04 TEMPERATURE · {temperature.toFixed(1)}</span><input className="temperature" type="range" min="0" max="2" step="0.1" value={temperature} onChange={e=>{setTemperature(Number(e.target.value));resetRun()}}/></label>}</div>
 {realUnavailable&&<div className="real-warning"><AlertTriangle size={16}/><b>{model.name} 暂未接入真实 API</b><span>V1 Real 模式目前仅支持 Qwen / 通义千问。</span></div>}{error&&<div className="real-error"><AlertTriangle size={16}/><span>{error}</span></div>}
 <div className="runner-grid"><div className="runner-col"><PanelTitle n="A" title="测试上下文" tag={tc.modality}/><InfoBlock label="用户输入" text={tc.inputText}/><InfoBlock label="期望答案" text={tc.expectedAnswer}/><InfoBlock label="评分标准" text={tc.evaluationCriteria}/></div><div className="runner-col"><PanelTitle n="B" title="当前 Prompt" tag={pv.name}/><pre className="prompt-view">{pv.content}</pre><div className="run-action"><button className="btn primary large" onClick={run} disabled={stage==='running'||stage==='judging'||realUnavailable}>{stage==='running'?<><Spinner/>{runMode==='real'?'Qwen 调用中...':'模型生成中...'}</>:<><Sparkles size={17}/>{runMode==='real'?'调用 Qwen 真实 API':'运行模拟测试'}</>}</button><small>{runMode==='real'?'请求由本地后端安全转发，API Key 不会发送到浏览器':'本次运行不会调用真实 AI API'}</small></div></div></div>
 <section className="output-panel"><PanelTitle n="C" title="模型输出与评测" tag={output?`${model.name} · ${latency} ms`:'等待运行'}/>{output&&(runMode==='mock'?<div className="mock-notice"><MockBadge/> 此输出和评分由前端规则模拟生成，未调用 {model.name} 或任何真实模型 API。</div>:<div className="real-notice"><RealBadge/> 输出来自 {apiIdentity?.provider} / {apiIdentity?.model} 真实调用；后续 AI 评分仍为 Mock 模拟评测。{usage&&<span>Token：{usage.totalTokens}（输入 {usage.promptTokens} / 输出 {usage.completionTokens}）</span>}</div>)}{!output&&stage!=='running'?<Empty text={`选择配置并运行${runMode==='real'?'真实 API':'模拟'}测试，输出将在这里出现`}/>:stage==='running'?<LoadingLines/>:<><div className="model-output">{output}</div>{!judgement?<div className="judge-action"><button className="btn secondary" onClick={judge} disabled={stage==='judging'}>{stage==='judging'?<><Spinner/>评测中...</>:<><CircleGauge size={17}/>AI 模拟评测</>}</button></div>:<Judgement value={judgement} save={doSave} saved={stage==='saved'}/>}</>}</section></>
}
...
function Judgement({value,save,saved}:any){return <div className="judgement"><div className="judge-score"><div className={`score-ring ${scoreTone(value.score)}`}><b>{value.score}</b><small>总分</small></div><div className="dimensions">{Object.entries(value.dimensionScores).map(([k,v])=><div key={k}><span>{({accuracy:'准确性',completeness:'完整性',format:'格式合规',usefulness:'有用性',safety:'风险控制'} as any)[k]}</span><i><em style={{width:`${v}%`}}/></i><b>{v as number}</b></div>)}</div></div><div className="judge-notes"><div><b>评审意见</b><p>{value.judgeComment}</p></div><div><b>优化建议</b><p>{value.suggestion}</p></div></div><div className="save-row"><span className="badge good">Bad Case：{value.badCaseType}</span><button className="btn primary" onClick={save} disabled={saved}>{saved?<><Check size={16}/>已保存</>:<>保存评测记录</>}</button></div></div>}
...
function BadCases({data,remove}:any){const bads=data.results.filter((r:EvalResult)=>r.score<70||r.badCaseType!=='无');return <><PageHead eyebrow="FAILURE ANALYSIS" title="Bad Case" desc="聚焦低分和异常输出，把失败样本转化为 Prompt 优化线索。"/><div className="bad-summary"><AlertTriangle size={18}/><b>{bads.length} 个待处理问题</b><span>判定规则：总分低于 70，或 Bad Case 类型不为“无”</span></div>{bads.length?<div className="bad-list">{bads.map((r:EvalResult)=><article key={r.id}><div className="bad-head"><div><div><span className="badge bad">{r.badCaseType}</span>{r.runMode==='mock'?<MockBadge/>:<RealBadge/>}</div><h3>{resultTitle(r)}</h3><small>{r.modelName} · {resultPrompt(r)} · {fmt(r.createdAt)}</small></div><div className="bad-actions"><div className={`score-box ${scoreTone(r.score)}`}><b>{r.score}</b><span>分</span></div><button className="btn delete-record" onClick={()=>{if(confirm('确定删除这条评测记录吗？删除后将从 Bad Case 和评测报告中移除。'))remove(r.id)}}><Trash2 size={14}/>删除记录</button></div></div><InfoBlock label="模型输出" text={r.modelOutput}/><div className="bad-notes"><div><b>评审意见</b><p>{r.judgeComment}</p></div><div><b>优化建议</b><p>{r.suggestion}</p></div></div></article>)}</div>:<Empty text="暂无 Bad Case，继续保持"/>}</>}
function Report({data}:any){const [filter,setFilter]=useState<'all'|'mock'|'real'>('all');const rs:EvalResult[]=data.results.filter((r:EvalResult)=>filter==='all'||r.runMode===filter);const dims=['accuracy','completeness','format','usefulness','safety'] as (keyof DimensionScores)[];const by=(getKey:(r:EvalResult)=>string)=>Object.entries(rs.reduce((a:any,r)=>{const key=getKey(r);(a[key]??=[]).push(r.score);return a},{})).map(([k,v])=>({key:k,score:avg(v as number[])})).sort((a,b)=>b.score-a.score);const bads=rs.filter(r=>r.score<70||r.badCaseType!=='无').length;const emptyText=filter==='real'?'暂无真实 API 评测记录':'保存评测记录后即可生成报告';return <><PageHead eyebrow="EVALUATION REPORT" title="评测报告" desc="汇总已保存记录，观察模型和 Prompt 版本的真实差异。" action={<div className="report-filter">{([['all','全部记录'],['mock','仅 Mock'],['real','仅真实']] as const).map(([id,label])=><button key={id} className={filter===id?'active':''} onClick={()=>setFilter(id)}>{label}</button>)}</div>}/>{!rs.length?<Empty text={emptyText}/>:<><section className="report-metrics"><Metric label="平均分" value={avg(rs.map(r=>r.score))} sub="当前筛选范围" icon={CircleGauge} tone="cyan"/><Metric label="通过率" value={`${Math.round(rs.filter(r=>r.score>=70).length/rs.length*100)}%`} sub="通过线 ≥ 70 分" icon={Check} tone="green"/><Metric label="总测试次数" value={rs.length} sub="当前筛选记录" icon={FlaskConical} tone="violet"/><Metric label="Bad Case" value={bads} sub="需要持续优化" icon={AlertTriangle} tone="orange"/></section>
 <div className="report-grid"><div className="panel"><div className="panel-title"><div><h3>各维度平均分</h3><p>五项核心评测指标</p></div></div><div className="bar-chart">{dims.map(k=>{const n=avg(rs.map(r=>r.dimensionScores[k]));return <div key={k}><span>{({accuracy:'准确性',completeness:'完整性',format:'格式合规',usefulness:'有用性',safety:'风险控制'} as any)[k]}</span><i><em style={{width:`${n}%`}}/></i><b>{n}</b></div>})}</div></div><Rank title="按模型统计" items={by(r=>r.modelName)} name={(k:string)=>k}/><Rank title="按 Prompt 版本统计" items={by(resultPrompt)} name={(k:string)=>k}/></div>
 <div className="panel recent-table"><div className="panel-title"><div><h3>最近评测记录</h3><p>按保存时间倒序排列</p></div></div><div className="table-wrap"><table><thead><tr><th>测试用例</th><th>模型</th><th>Prompt</th><th>数据来源</th><th>Bad Case</th><th>得分</th><th>时间</th></tr></thead><tbody>{rs.slice().sort((a,b)=>+new Date(b.createdAt)-+new Date(a.createdAt)).slice(0,8).map(r=><tr key={r.id}><td><b>{resultTitle(r)}</b></td><td>{r.modelName}</td><td>{resultPrompt(r)}</td><td>{r.runMode==='mock'?<MockBadge/>:<span className="badge good">真实 API</span>}</td><td><span className={`badge ${r.badCaseType==='无'?'good':'bad'}`}>{r.badCaseType}</span></td><td><strong className={scoreTone(r.score)}>{r.score}</strong></td><td>{fmt(r.createdAt)}</td></tr>)}</tbody></table></div></div></>}</>}
...
function Modal({title,close,children}:any){return <div className="modal-backdrop" onMouseDown={e=>{if(e.target===e.currentTarget)close()}}><div className="modal"><div className="modal-head"><h3>{title}</h3><button className="icon-btn" onClick={close}><X size={20}/></button></div>{children}</div></div>}
