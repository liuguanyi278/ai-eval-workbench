...
function Runner({data,save}:any){
 ...
 <section className="output-panel"><PanelTitle n="C" title="模型输出与评测" tag={output?`${modelDisplayName} · ${latency} ms`:'等待运行'}/>{output&&(runMode==='mock'?<div className="mock-notice"><MockBadge/> 此输出和评分由前端规则模拟生成，未调用 {modelDisplayName} 或任何真实模型 API。</div>:<div className="real-notice"><RealBadge/> 输出来自 {modelDisplayName} 真实调用；后续 AI 评分仍为 Mock 模拟评测。{usage&&<span>Token：{usage.totalTokens}（输入 {usage.promptTokens} / 输出 {usage.completionTokens}）</span>}</div>)}{!output&&stage!=='running'?<Empty text={`选择配置并运行${runMode==='real'?'真实 API':'模拟'}测试，输出将在这里出现`}/>:stage==='running'?<LoadingLines/>:<><div className="model-output">{output}</div>{!judgement?<div className="judge-action"><button className="btn secondary" onClick={judge} disabled={stage==='judging'}>{stage==='judging'?<><Spinner/>评测中...</>:<><CircleGauge size={17}/>AI 模拟评测</>}</button></div>:<Judgement value={judgement} save={doSave} saved={stage==='saved'}/>}</>}</section></>
}
...
function ReportTable({data}:any){
 ...
 const recent=rs.filter(r=>caseFilter==='all'||resultTitle(r)===caseFilter).filter(r=>modelFilter==='all'||resultModel(r)===modelFilter).filter(r=>promptFilter==='all'||resultPrompt(r)===promptFilter).filter(r=>tempFilter==='all'||tempLabel(r)===tempFilter).filter(r=>sourceFilter==='all'||r.runMode===sourceFilter).filter(r=>badFilter==='all'||(badFilter==='bad'?isBad(r):!isBad(r))).filter(r=>scoreFilter==='all'||(scoreFilter==='low'?r.score<70:scoreFilter==='medium'?r.score>=70&&r.score<=84:r.score>=85)).sort((a,b)=>(sortOrder==='newest'?1:-1)*(+new Date(b.createdAt)-+new Date(a.createdAt)))
...
function RecordDetail({record:r,close}:{record:EvalResult;close:()=>void}){const d=r.dimensionScores||{} as DimensionScores;const usage=r.tokenUsage||r.usage;const value=(x?:string)=>x||'未记录';return <Modal title="评测记录详情" close={close} wide><div className="detail-section"><h4>基本信息</h4><div className="detail-grid"><DetailItem label="测试用例标题" value={resultTitle(r)}/><DetailItem label="Provider / Model" value={resultModel(r)}/><DetailItem label="Prompt 版本" value={resultPrompt(r)}/><DetailItem label="数据来源" value={r.runMode==='real'?'Real 真实 API':'Mock 模拟数据'}/><DetailItem label="得分" value={`${r.score ?? '—'} 分`}/><DetailItem label="Bad Case 类型" value={r.badCaseType||'无'}/><DetailItem label="创建时间" value={new Date(r.createdAt).toLocaleString('zh-CN')}/></div></div><div className="detail-section"><h4>测试用例快照</h4><InfoBlock label="用户输入" text={value(r.testCaseInputSnapshot)}/><InfoBlock label="期望答案" text={value(r.expectedAnswerSnapshot)}/><InfoBlock label="评分标准" text={value(r.evaluationCriteriaSnapshot)}/></div><div className="detail-section"><h4>Prompt 快照</h4><DetailItem label="Prompt 名称" value={resultPrompt(r)}/><InfoBlock label="Prompt 内容" text={value(r.promptContentSnapshot)}/></div><div className="detail-section"><h4>模型输出</h4><div className="detail-output">{value(r.modelOutput)}</div></div><div className="detail-section"><h4>评分结果</h4><div className="detail-grid score-details"><DetailItem label="总分" value={r.score??'—'}/><DetailItem label="准确性" value={d.accuracy??'—'}/><DetailItem label="完整性" value={d.completeness??'—'}/><DetailItem label="格式合规" value={d.format??'—'}/><DetailItem label="有用性" value={d.usefulness??'—'}/><DetailItem label="风险控制" value={d.safety??'—'}/></div><InfoBlock label="评审意见" text={value(r.judgeComment)}/><InfoBlock label="优化建议" text={value(r.suggestion)}/></div><div className="detail-section"><h4>调用元数据</h4><div className="detail-grid"><DetailItem label="temperature" value={typeof r.temperature==='number'?r.temperature:'未记录'}/><DetailItem label="latencyMs" value={`${r.latencyMs??'未记录'} ms`}/><DetailItem label="tokenUsage" value={usage?`输入 ${usage.promptTokens} / 输出 ${usage.completionTokens} / 总计 ${usage.totalTokens}`:'未记录'}/><DetailItem label="provider" value={r.provider||r.providerName||r.modelName||'未记录'}/><DetailItem label="actualModel" value={r.actualModel||r.apiModel||'未记录'}/></div></div></Modal>}
